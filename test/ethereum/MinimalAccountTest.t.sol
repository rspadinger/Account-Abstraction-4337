...
contract MinimalAccountTest is Test, ZkSyncChainChecker {
    ...
    // approve some amount
    // USDC contract
    // come from the entrypoint ??? => we are not coming from the entrypoint => this is executed by the MinAccount owner
    ...
    function testNonOwnerCannotExecuteCommands() public skipZkSync {
        ...
        // Act
        vm.prank(randomuser);
        //@note revert with custom error using error sellector
    ...
    function testEntryPointCanExecuteCommands() public skipZkSync {
        ...
        //@note Act => this can be a random user - eg: any alt mempool validator
        vm.prank(randomuser);
        //handleOps : ops[] , beneficiary => here, it is ourmempool validator
