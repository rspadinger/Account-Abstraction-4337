 ...
 * msg.sender is the bootloader system contract => like a superAdmin - acts like the EntryPoint contract on Ethereum
 *
 * Phase 1 Validation
...
contract ZkMinimalAccount is IAccount, Ownable {
    ...
    //no AA, just send a signed txn to someone & that person calls this function pays for txn
    ...
    function _executeTransaction(Transaction memory _transaction) internal {
        //we need to cast, because to in transaction is of type uint256
        address to = address(uint160(_transaction.to));
        //we need to cast this to uint128, because a system contract may require a uint128 for the value
        ...
        if (to == address(DEPLOYER_SYSTEM_CONTRACT)) {
            ...
            bool success;
            //call, delegatecall... work slightly different in zksync => that's why we use assembly here
            assembly {
